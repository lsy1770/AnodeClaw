...
declare const auto: {
  ...
  // Chainable: .text(s) .textContains(s) .id(s) .desc(s) .className(s) .clickable(b) .scrollable(b) .editable(b) .visible(b) .enabled(b)
  ...
  // Kotlin: waitFor(selector) — always 10000ms; waitForWithTimeout(selector, timeout) for custom
  waitFor(selector: any): Promise<any>;
  waitForWithTimeout(selector: any, timeout: number): Promise<any>;
  // Kotlin: waitForGone(selector: AccessibilitySelector) / waitForGoneWithTimeout(selector, timeout)
  waitForGone(selector: any): Promise<boolean>;
  waitForGoneWithTimeout(selector: any, timeout: number): Promise<boolean>;
...
function getNodeBounds(bounds: any): { left: number; top: number; right: number; bottom: number } | null {
...
async function clickNodeWithFallback(node: any): Promise<boolean> {
...
async function findScreenCapturePermissionNode(label: string): Promise<any | null> {
...
async function autoApproveScreenCapturePermissionDialog(timeoutMs = 8000): Promise<{
  clicked: boolean;
  label?: string;
  attempts: number;
}> {
...
async function requestScreenCapturePermissionWithAutoApproval(timeoutMs = 8000): Promise<{
  requestResult: any;
  ...
  const approvalPromise = autoApproveScreenCapturePermissionDialog(timeoutMs).catch((error) => {
    ...
    return { clicked: false, attempts: 0 } as { clicked: boolean; label?: string; attempts: number };
  });
...
function isScreenCapturePermissionError(error: unknown): boolean {
  const message = getErrorMessage(error).toLowerCase();
  return /permission|media.?projection|screen capture|capture screen|not granted|denied|consent/.test(message);
}

async function isAccessibilityScreenshotSupported(): Promise<boolean> {
...
export const androidFindTextTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const nodeInfos = limitedNodes.map((node: any) => {
        try {
          return {
            text: node.text || text,
            bounds: node.bounds,
...
export const androidScreenshotTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      // Capture screenshot
      let bitmap: any;
      let permissionRetry: { autoApproved: boolean; clickedLabel?: string; attempts: number } | undefined;
      if (useAccessibility) {
 ...
 */
export const androidPressTool: Tool = {
  name: 'android_press',
...
const androidGesturePointsSchema = z.preprocess(
  (value) => {
...
export const androidGestureTool: Tool = {
 ...
 */
export const androidGesturesTool: Tool = {
  ...
  parameters: [
    {
      ...
      schema: z.preprocess(
        (value) => {
          if (!Array.isArray(value)) return value;
          return value.map((stroke: any) => ({
            ...stroke,
            points: Array.isArray(stroke?.points)
              ? stroke.points.map((point: any) => {
                  if (Array.isArray(point) && point.length >= 2) {
                    return { x: point[0], y: point[1] };
                  }
                  return point;
 ...
 */
export const androidQuickSettingsTool: Tool = {
 ...
 */
export const androidAppendTextTool: Tool = {
 ...
 */
export const androidScrollToTool: Tool = {
 ...
 */
export const androidGetCurrentPackageTool: Tool = {
 ...
 */
export const androidGetCurrentActivityTool: Tool = {
 ...
 */
export const androidScreenStateTool: Tool = {
 ...
 */
export const androidExistsTool: Tool = {
  name: 'android_exists',
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const selector = auto.selector();
      if (text) {
        if (exact) { selector.text(text); } else { selector.textContains(text); }
      }
 ...
 */
export const androidFindOneTool: Tool = {
 ...
 */
export const androidWaitForGoneTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const selector = auto.selector();
      if (text) { selector.textContains(text); }
      if (id) { selector.id(id); }
 ...
 */
export const androidGetWindowsTool: Tool = {
 ...
 */
export const androidGetLayoutTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const filteredNodes = limitedNodes.filter(node => {
        ...
        if (interactiveOnly && !node.clickable && !node.scrollable && !node.editable) return false;
        return true;
      });
 ...
 */
export const androidFindInteractiveElementsTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const addNode = (node: any, type: string) => {
        if (!node) return;
 ...
 */
export const androidDescribeScreenTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      const interactiveMap = new Map<string, any>();

      const addInteractive = (node: any) => {
        if (!node || !node.visible) return;  // 只要可见元素
        const key = `${node.bounds?.left}-${node.bounds?.top}-${node.className}`;
      ...
      return {
        ...
        output: {
          ...
          windows: {
            ...
            active: windows.find((w: any) => w.isActive),
          },
          interactiveElements: {
 ...
 */
export const androidClickElementTool: Tool = {
 ...
 */
export const androidLongClickElementTool: Tool = {
 ...
 */
export const androidInputTextToElementTool: Tool = {
 ...
 */
export const androidScrollElementTool: Tool = {
 ...
 */
export const androidExpandElementTool: Tool = {
 ...
 */
export const androidCollapseElementTool: Tool = {
 ...
 */
export const androidFocusElementTool: Tool = {
