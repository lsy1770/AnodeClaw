...
// Anode file global (based on anode-api.d.ts)
declare const file: {
  ...
  copy(sourcePath: string, targetPath: string): Promise<any>;
  move(sourcePath: string, targetPath: string): Promise<any>;
  readText(path: string, charset?: string | null): Promise<string>;
  ...
  listFilesRecursively(path: string): Promise<any[]>;
  getFileInfo(path: string): Promise<any>;
  getMd5(path: string): Promise<string>;
  encodeBase64(path: string): Promise<string>;
...
export const listFilesTool: Tool = {
  ...
  async execute(params, options): Promise<ToolResult> {
    try {
      ...
      // Extract file names
      const files = fileInfoList.map((f: any) => f.name);
