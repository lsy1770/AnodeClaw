...
export type ToolStrategyMode = 'always' | 'auto' | 'off';

export interface ToolUsageDecision {
  shouldIncludeTools: boolean;
  reasoning: string;
  /** Optional category filter — only include tools from these categories */
  toolFilter?: string[];
}
...
export class ToolUsageStrategy {
   ...
   */
  static analyzeMessage(message: string, mode: ToolStrategyMode = 'auto'): ToolUsageDecision {
    // Mode overrides
    if (mode === 'always') {
   ...
   */
  static inferToolCategories(message: string): string[] {
