...
export interface EmbeddingProviderConfig {
  ...
  preferredProvider?: 'openai' | 'gemini' | 'local';
}
...
export class LocalEmbeddingProvider implements EmbeddingProvider {
  readonly name = 'local';
  readonly model = 'tfidf-v1';
  readonly dimensions = 1000; // Fixed vocabulary size
  ...
  constructor(config?: EmbeddingProviderConfig) {
    ...
    if (config?.preferredProvider) {
      this.activeProvider = this.providers.find(
        p => p.name === config.preferredProvider && p.isAvailable()
      ) || null;
    }
 ...
 */
export function createEmbeddingManager(config?: EmbeddingProviderConfig): EmbeddingManager {
