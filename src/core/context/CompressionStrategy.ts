...
import type { CompressionOptions, CompressionResult, CompressionStrategy } from './types.js';
import { TokenCounter } from './TokenCounter.js';
import { logger } from '../../utils/logger.js';
import { ModelAPI } from '../ModelAPI.js';

/** Optional AI model for generating summaries */
let _modelAPI: ModelAPI | null = null;
let _summaryModel: string = 'claude-3-haiku-20240307';

/**
 * Set the ModelAPI instance for AI-powered compression
 */
export function setCompressionModelAPI(modelAPI: ModelAPI, model?: string): void {
  _modelAPI = modelAPI;
  if (model) _summaryModel = model;
...
export class CompressionStrategyHandler {
  ...
  static async compress(
    ...
    logger.info(
      `[Compression] Strategy: ${options.strategy}, ` +
   ...
   * @returns Compression result
   */
  private static async summaryStrategy(
    messages: Message[],
    options: CompressionOptions
  ): Promise<CompressionResult> {
  ...
  private static pruneStrategy(
    ...
    if (options.preserveImportant) {
      ...
      const important = messages.filter(
        (m) => m.metadata?.important || m.role === 'system' || this.hasMultimodalContent(m)
      );
   ...
   * @returns Compression result
   */
  private static async hybridStrategy(
    messages: Message[],
    options: CompressionOptions
  ): Promise<CompressionResult> {
   ...
   */
  static hasMultimodalContent(message: Message): boolean {
   ...
   */
  private static async generateAISummary(messages: Message[]): Promise<string> {
      ...
      .map(m => {
        if (this.hasMultimodalContent(m)) {
          ...
          for (const b of blocks) {
            ...
            else if (b.type === 'file') parts.push(`[File: ${b.source?.filename || 'unknown'}]`);
          }
   ...
   */
  private static generateNaiveSummary(messages: Message[]): string {
    ...
    return (
      `This conversation covered ${messages.length} messages over ${topics.length} topics:\n` +
      ...
      if (message.role === 'user') {
        ...
        if (typeof message.content === 'string') {
          ...
          text = (message.content as any[])
            ...
            .map((b: any) => b.text)
            .join(' ');
          if ((message.content as any[]).some((b: any) => b.type === 'image')) {
            text += ' [with image]';
          }
        } else {
          continue;
