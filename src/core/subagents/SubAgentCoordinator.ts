...
export class SubAgentCoordinator extends EventEmitter {
  ...
  async executeWorkflow(workflow: Workflow): Promise<WorkflowResult> {
    logger.info(
      `[SubAgentCoordinator] Executing workflow: ${workflow.name} (${workflow.tasks.length} tasks)`
    );
    ...
    try {
      ...
      const taskResults = new Map<string, AgentTaskResult>();
      ...
      while (remainingTasks.size > 0) {
        ...
        if (workflow.parallel) {
          ...
          const results = await Promise.all(
            readyTasks.map((task) => this.executeTask(task.agentId, task))
          );
      ...
      logger.info(
        `[SubAgentCoordinator] Workflow ${workflow.name} completed in ${duration}ms (success: ${allSuccess})`
      );
  ...
  sendMessage(
    ...
    to: string,
    content: any,
    type: 'request' | 'response' | 'notification' = 'notification'
  ): string {
    ...
    logger.debug(
      `[SubAgentCoordinator] Message ${message.id} from ${from} to ${to}`
    );
