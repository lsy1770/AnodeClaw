...
/**
 * Thinking content extracted from stream
 */
export interface ThinkingContent {
  /** Raw thinking text (inside <think> tags) */
  thinking: string;
  /** Clean content (thinking removed) */
  content: string;
  /** Whether thinking is complete (closed tag found) */
  isComplete: boolean;
}
...
export class DeltaBuffer {
  ...
  constructor(blockSize: number = 2000) {
   ...
   */
  appendDedup(fullContent: string): string {
    if (!fullContent) return this.buffer;
   ...
   * Pattern: <think>...</think>
   */
  extractThinking(): ThinkingContent {
   ...
   */
  splitBlocks(customBlockSize?: number): BlockSplitResult {
    ...
    while (pos + size <= content.length) {
      ...
      if (localBreak === -1) {
        ...
        if (localBreak !== -1) localBreak += 1; // Include the period
      }
      if (localBreak === -1) {
        localBreak = searchArea.lastIndexOf(' ');
      }
 ...
 */
export function createDeltaBuffer(blockSize?: number): DeltaBuffer {
