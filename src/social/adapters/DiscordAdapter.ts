...
async function dynamicImport(moduleName: string): Promise<any> {
  const script = new Script(
    `import("${moduleName}")`,
    { importModuleDynamically: (constants as any).USE_MAIN_CONTEXT_DEFAULT_LOADER }
  );
...
async function getDiscordJS(): Promise<any> {
  if (DiscordJS !== null) {
    return DiscordJS;
  }
...
export class DiscordAdapter extends BaseSocialAdapter {
  ...
  protected async connect(): Promise<void> {
    ...
    if (!discordLib || !discordLib.Client) {
      throw new Error(
        'discord.js not available. Install: npm install discord.js'
      );
    }
    ...
    this.client.on('ready', () => {
      ...
      logger.info(
        `[${this.platformName}] Bot connected as ${this.botUser?.tag} (${this.botUser?.id})`
      );
    });
  ...
  async sendMessageWithId(message: OutgoingMessage): Promise<string> {
    ...
    try {
      ...
      if (message.attachments && message.attachments.length > 0) {
        options.files = message.attachments.map((att: any) => ({
          attachment: att.url,
  ...
  async getChatInfo(chatId: string): Promise<ChatInfo> {
    ...
    return {
      ...
      title: (channel as any).name || 'Direct Message',
      memberCount: isGroup ? (channel as any).members?.size : undefined,
      platform: this.platformName,
    };
  }
  ...
  private async handleDiscordMessage(msg: any): Promise<void> {
    try {
      ...
      logger.debug(
        `[${this.platformName}] Message received: isDM=${isDM}, isMentioned=${isMentioned}, ` +
  ...
  private extractAttachments(msg: any): any[] {
    ...
    if (msg.embeds && msg.embeds.length > 0) {
      for (const embed of msg.embeds) {
        if (embed.image) attachments.push({ type: 'image', url: embed.image.url });
        if (embed.video) attachments.push({ type: 'video', url: embed.video.url });
      }
    }
