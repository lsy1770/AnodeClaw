...
declare const floatingWindow: {
  ...
  on(eventType: string, callback: (data: any) => void): boolean;
  onView(viewId: string, eventType: string, callback: (data: any) => void): boolean;
};
...
export class SettingsPanel {
  ...
  // Event handlers
  private onSettingsChangedCallback?: (config: Config) => void;
   ...
   */
  async show(config: SettingsPanelConfig = {}): Promise<void> {
    if (this.isVisible) {
   ...
   */
  onSettingsChanged(callback: (config: Config) => void): void {
  ...
  private setupEventListeners(): void {
    ...
    const clickSuccess = floatingWindow.on('click', (event: any) => {
      try {
    ...
    const textChangedSuccess = floatingWindow.on('textChanged', (event: any) => {
...
<LinearLayout
   ...
   * Handle tool toggle
   */
  private handleToolToggle(toolName: string): void {
    try {
