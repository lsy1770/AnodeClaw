...
export class ControlPanel {
  ...
  private async handleSaveConfig(): Promise<void> {
    try {
      ...
      if (tgToken) config.social.telegram = { enabled: this.getCheckboxState('check_telegram_enabled'), botToken: tgToken };

      const qqId = this.readText('edit_qq_appid');
      const qqTok = this.readText('edit_qq_token');
      if (qqId || qqTok) config.social.qq = { enabled: this.getCheckboxState('check_qq_enabled'), appId: qqId || undefined, token: qqTok || undefined };

      config.social.wechat = { enabled: this.getCheckboxState('check_wechat_enabled') };

      const dcTok = this.readText('edit_discord_token');
      if (dcTok) config.social.discord = { enabled: this.getCheckboxState('check_discord_enabled'), botToken: dcTok };

      const fsId = this.readText('edit_feishu_appid');
      const fsSec = this.readText('edit_feishu_secret');
      if (fsId || fsSec) config.social.feishu = { enabled: this.getCheckboxState('check_feishu_enabled'), appId: fsId || undefined, appSecret: fsSec || undefined };

      const dtKey = this.readText('edit_dingtalk_appkey');
      const dtSec = this.readText('edit_dingtalk_secret');
      if (dtKey || dtSec) config.social.dingtalk = { enabled: this.getCheckboxState('check_dingtalk_enabled'), appKey: dtKey || undefined, appSecret: dtSec || undefined };
