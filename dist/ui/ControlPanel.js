...
export class ControlPanel {
     ...
     */
    setupEventListeners() {
        const success = floatingWindow.on('click', (event) => {
            try {
                switch (event.viewId) {
                    case 'btn_close':
                        this.close();
    ...
    async loadConfigToUI() {
        try {
            ...
            logger.error('Failed to load config to UI', { error: error.message });
        }
    }
    /**
     * 保存配置
    ...
    async handleSaveConfig() {
        try {
            ...
            this.updateStatus('❌ 配置保存失败: ' + error.message);
        }
    }
    /**
     * 启动聊天窗口
    ...
    async handleStartChat() {
        ...
        try {
            ...
            logger.error('Failed to start chat', { error: error.message });
    ...
    handleStopChat() {
        ...
        try {
            ...
            logger.error('Failed to stop chat', { error: error.message });
            this.updateStatus('❌ 停止失败: ' + error.message);
        }
    }
    /**
     * 更新状态显示
