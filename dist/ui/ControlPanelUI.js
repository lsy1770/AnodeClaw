...
export class ControlPanel {
    ...
    loadConfigToUI() {
        try {
            ...
            this.setProviderRadio(provider);
            ...
            if (config.social) {
                if (config.social.telegram) {
                    ...
                    if (config.social.telegram.botToken)
                        this.writeText('edit_telegram_token', config.social.telegram.botToken);
                }
                if (config.social.qq) {
                    this.setCheckboxState('check_qq_enabled', config.social.qq.enabled ?? false);
                    if (config.social.qq.appId)
                        this.writeText('edit_qq_appid', config.social.qq.appId);
                    if (config.social.qq.token)
                        this.writeText('edit_qq_token', config.social.qq.token);
                }
                ...
                if (config.social.discord) {
                    ...
                    if (config.social.discord.botToken)
                        this.writeText('edit_discord_token', config.social.discord.botToken);
                }
                if (config.social.feishu) {
                    this.setCheckboxState('check_feishu_enabled', config.social.feishu.enabled ?? false);
                    if (config.social.feishu.appId)
                        this.writeText('edit_feishu_appid', config.social.feishu.appId);
                    if (config.social.feishu.appSecret)
                        this.writeText('edit_feishu_secret', config.social.feishu.appSecret);
                }
                if (config.social.dingtalk) {
                    this.setCheckboxState('check_dingtalk_enabled', config.social.dingtalk.enabled ?? false);
                    if (config.social.dingtalk.appKey)
                        this.writeText('edit_dingtalk_appkey', config.social.dingtalk.appKey);
                    if (config.social.dingtalk.appSecret)
                        this.writeText('edit_dingtalk_secret', config.social.dingtalk.appSecret);
                }
            }
            logger.info('[ControlPanel] config → UI loaded');
        }
        catch (error) {
            logger.error('[ControlPanel] loadConfigToUI failed', { error: error.message });
        }
    }
    async handleSaveConfig() {
        try {
            const config = this.configManager.get();
            // Provider
            if (this.getCheckboxState('radio_openai'))
                config.model.provider = 'openai';
            else if (this.getCheckboxState('radio_gemini'))
            ...
            if (maxTok)
                config.model.maxTokens = parseInt(maxTok, 10);
            ...
            if (ctxWarn)
                config.agent.contextWindowWarning = parseInt(ctxWarn, 10);
            // Advanced checkboxes
            ...
            logger.error('[ControlPanel] save failed', { error: error.message });
            globalApi.toast('配置保存失败: ' + error.message, "long");
        }
    }
    // ==================================================
    // Chat window
    ...
    async handleStartChat() {
        ...
        try {
            ...
            globalApi.toast('启动失败: ' + error.message, "long");
        }
    }
    handleStopChat() {
        if (!this.isChatRunning)
        ...
        try {
            ...
            logger.error('[ControlPanel] stop chat failed', { error: error.message });
        }
    }
    // ==================================================
