...
export default class CalculatorPlugin {
    ...
    getTools() {
        return [
            {
                name: 'calculate',
                description: 'Evaluate a mathematical expression (supports +, -, *, /, ^, sqrt, sin, cos, tan, log, etc.)',
                parameters: [
                    {
                        name: 'expression',
    ...
    async convertUnits(params) {
        ...
        const precision = this.context.pluginConfig.settings.precision || 4;
        try {
            const result = this.performUnitConversion(parsed.value, parsed.fromUnit.toLowerCase(), parsed.toUnit.toLowerCase());
            const rounded = Number(result.toFixed(precision));
            this.context.log('info', `Converted: ${parsed.value} ${parsed.fromUnit} = ${rounded} ${parsed.toUnit}`);
            return JSON.stringify({
                value: parsed.value,
