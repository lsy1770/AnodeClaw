...
export class PluginLoader {
    ...
    async load(pluginId) {
        ...
        try {
            ...
            try {
                ...
                return {
                    success: false,
                    error: `Failed to import plugin module (dynamic import may not be supported on this runtime): ${importError.message}`,
                };
            }
    ...
    async loadAll() {
        ...
        try {
            ...
            const pluginDirs = entries.filter((entry) => entry.type === 'directory' && !entry.name.includes('.'));
     ...
     */
    async createPluginDir(pluginId) {
        const pluginPath = this.getPluginPath(pluginId);
