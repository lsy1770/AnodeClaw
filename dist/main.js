 ...
 * 确保所有必要的数据目录存在
 */
async function ensureDataDirectories(config) {
    const dirs = [
 ...
 * 创建ClawdBot实例
 */
async function createClawdBot(configPath = null) {
    try {
        // 1. 加载配置
        ...
        logger.error('Failed to initialize ClawdBot', { error: error.message });
        throw error;
    }
}
/**
 ...
 * 处理命令
 */
async function handleCommand(cmd, bot, session) {
