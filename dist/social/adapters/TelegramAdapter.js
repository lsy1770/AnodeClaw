...
const TelegramBot = TelegramBotLib;
/**
 * Telegram adapter
 */
export class TelegramAdapter extends BaseSocialAdapter {
    ...
    async connect() {
        ...
        try {
            ...
            logger.error(`[${this.platformName}] Failed to create TelegramBot instance. ` +
                `Ensure node-telegram-bot-api is installed: npm install node-telegram-bot-api`, error);
            throw new Error('Failed to create TelegramBot instance. Is node-telegram-bot-api installed?');
        }
        this.bot = bot;
        ...
        try {
            ...
            logger.info(`[${this.platformName}] Bot connected as @${me.username} (${me.id})`);
        }
        catch (error) {
            logger.error(`[${this.platformName}] Failed to validate bot token via getMe():`, error);
            ...
            this.bot = undefined;
            throw new Error('Telegram bot token validation failed. Check your token and network connectivity.');
        }
    }
    /**
                ...
                for (const attachment of message.attachments) {
                    switch (attachment.type) {
                        case 'image':
                            ...
                            break;
                        case 'file':
                            await this.bot.sendDocument(message.chatId, attachment.url, options);
                            break;
                        case 'audio':
