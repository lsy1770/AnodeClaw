...
const { createOpenAPI, createWebsocket } = QQGuildSDK;
import WS from 'ws';
const { WebSocket } = WS;
/**
...
export class QQAdapter extends BaseSocialAdapter {
    ...
    async sendMessage(message) {
        ...
        if (message.attachments && message.attachments.length > 0) {
            const segments = [{ type: 'text', data: { text: message.text } }];
    ...
    async handleQQMessage(data) {
        try {
            ...
            if (isGroup) {
                message.groupId = String(data.group_id);
                message.groupName = data.sender?.card || undefined;
            }
...
export class QQGuildAdapter extends BaseSocialAdapter {
    constructor() {
        super(...arguments);
        this.platformName = 'qq-guild';
        this.displayName = 'QQ Guild';
    }
    /**
     * Connect to QQ Guild
     */
    async connect() {
        ...
        try {
            ...
            logger.error(`[${this.platformName}] Failed to load qq-guild-bot. Install it with: npm install qq-guild-bot`);
            throw new Error('qq-guild-bot package not found. Please install it first.');
        }
    }
    /**
     ...
     */
    async handleQQGuildMessage(data) {
        try {
            const message = {
                messageId: data.id,
