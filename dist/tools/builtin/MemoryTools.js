...
let store = null;
export function setMemoryStore(instance) {
    store = instance;
...
function getStore() {
    if (!store)
        throw new Error('MemoryStore not initialized. Call setMemoryStore() first.');
    return store;
}
// ─── memory_search ────────────────────────────────────────────────────────────
...
export const memoryGetTool = {
    ...
    async execute(params) {
        ...
        try {
            ...
            if (!entry)
                return { success: false, error: { code: 'MEMORY_NOT_FOUND', message: `Memory "${params.id}" not found` }, metadata: { duration: Date.now() - t, toolName: 'memory_get', timestamp: Date.now() } };
            return {
                success: true,
...
export const memoryDeleteTool = {
    ...
    async execute(params) {
        ...
        try {
            ...
            if (!ok)
                return { success: false, error: { code: 'MEMORY_NOT_FOUND', message: `Memory "${params.id}" not found` }, metadata: { duration: Date.now() - t, toolName: 'memory_delete', timestamp: Date.now() } };
...
// ─── context_checkpoint ───────────────────────────────────────────────────────
export const contextCheckpointTool = {
