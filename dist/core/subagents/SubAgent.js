...
export class SubAgent extends EventEmitter {
    ...
    async executeTask(task) {
        ...
        logger.info(`[SubAgent:${this.config.id}] Starting task ${task.id}: ${task.instruction}`);
        this.status = 'working';
        ...
        try {
            ...
            logger.info(`[SubAgent:${this.config.id}] Task ${task.id} completed in ${duration}ms`);
            return result;
        }
        catch (error) {
            const endTime = Date.now();
            ...
            logger.error(`[SubAgent:${this.config.id}] Task ${task.id} failed:`, error);
            return result;
        }
    }
    /**
