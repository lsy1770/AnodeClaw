...
export class SystemPromptBuilder {
    constructor(params) {
        this.params = params;
        this.enabledSections = new Set(params.enabledSections || ['identity', 'safety', 'tooling', 'memory', 'workspace', 'projectContext', 'runtime']);
    }
    build() {
        const lines = [];
...
- Use sub-agents (\`create_subagent\` / \`delegate_subagent_task\`) only for genuinely parallel or specialized workloads`;
    ...
    buildSkillsSection() {
        ...
        if (!skills?.enabled || skills.skillNames.length === 0)
            return null;
...
- Never persist sensitive data (passwords, tokens, PII)`;
    ...
    // Public API
    enableSection(section) { this.enabledSections.add(section); }
    disableSection(section) { this.enabledSections.delete(section); }
    isSectionEnabled(section) { return this.enabledSections.has(section); }
...
export function getDefaultIdentity() { return DEFAULT_IDENTITY; }
export function getDefaultSafetyRules() { return [...DEFAULT_SAFETY_RULES]; }
