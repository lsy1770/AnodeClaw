...
export class PromptBuilder {
    ...
    async initialize() {
        ...
        logger.info(`[PromptBuilder] Loaded ${this.sections.size} prompt sections`);
     ...
     */
    async buildPrompt(variables = {}, contextPrompt) {
    ...
    startHotReload() {
        ...
        logger.info(`[PromptBuilder] Hot reload enabled (interval: ${this.config.reloadInterval}ms)`);
    }
    /**
     * Stop hot reload
     */
     ...
     */
    setSectionEnabled(source, enabled) {
        const section = this.sections.get(source);
        if (section) {
            section.enabled = enabled;
            logger.info(`[PromptBuilder] Section '${source}' ${enabled ? 'enabled' : 'disabled'}`);
        }
    }
    /**
     ...
     */
    addSection(source, content, priority) {
        this.sections.set(source, {
            source: source,
            content,
            priority,
