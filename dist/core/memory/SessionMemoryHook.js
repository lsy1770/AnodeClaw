...
export class SessionMemoryHook {
     ...
     */
    shouldCreateMemory(summary, turnCount) {
        ...
        const hasContent = summary.topics.length > 0 ||
     ...
     */
    async createMemoryFromSession(summary) {
     ...
     */
    async onSessionEnd(summary, turnCount) {
     ...
     */
    async onNewSession(currentSummary, turnCount) {
 ...
 */
export function createSessionMemoryHook(onMemorySave, config) {
 ...
 */
export function createEmptySessionSummary(sessionId, startTime) {
    return {
        sessionId,
        startTime,
        endTime: Date.now(),
